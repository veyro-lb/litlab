...
function teacherSupervisedUpload(data:Pipeline){
  ...
  helper.innerHTML='<div><b>Teacher approval chooses the final admin-review version</b><span>Submit the version you want your teacher to review. If the teacher approves it, that exact DOCX becomes the version LitLab admin reviews next. You do not need a separate “final” upload.</span></div>';
}
...
function studentHandoff(data:Pipeline){
  ...
  host.querySelector('[data-final-review-handoff-note]')?.remove();
  if(data.stage!=='admin_review'||!data.mentor_required||!data.final_review_document)return;
  const doc=data.final_review_document;
  const path=host.querySelector<HTMLElement>('[data-review-lifecycle-path]');if(!path)return;
  const note=document.createElement('div');note.dataset.finalReviewHandoffNote='true';note.className='ll-final-review-handoff-note';
...
function teacherHandoff(){
  ...
  const host=document.querySelector<HTMLElement>('[data-contributor-workspace]');if(!host)return;
  host.querySelectorAll<HTMLElement>('[data-teacher-final-handoff]').forEach(el=>el.remove());
  for(const assignment of assignments){
    const finalDoc=assignment.final_review_document;if(!finalDoc?.id)continue;
    const card=host.querySelector<HTMLElement>(`.ll-teacher-assignment[data-teacher-student-id="${CSS.escape(assignment.application_id)}"],.ll-teacher-assignment`);
    if(!card)continue;
    ...
    if(target){const badge=document.createElement('strong');badge.dataset.teacherFinalHandoff='true';badge.className='ll-teacher-final-doc-badge';badge.textContent='APPROVED → ADMIN FINAL REVIEW';target.appendChild(badge)}
    const form=card.querySelector<HTMLFormElement>('form[data-teacher-review],form.ll-review-form');
    const history=card.querySelector<HTMLElement>('.ll-review-history');
    const note=document.createElement('div');note.dataset.teacherFinalHandoff='true';note.className='ll-teacher-final-handoff-note';note.innerHTML=`<b>Final handoff complete</b><span>You approved <strong>${esc(finalDoc.original_name||finalDoc.version_label||'this DOCX')}</strong>. LitLab admin is reviewing that exact version now. No further teacher action is needed unless a revised DOCX returns to you.</span>`;
    (history||form||card.firstElementChild)?.before(note);
  }
}
...
function finalAdminCard(data:AdminWorkspace,applicationId:string){
  ...
  grid.querySelector('[data-admin-final-review-document]')?.remove();
  const finalDoc=data.final_review_document;
  const panel=document.createElement('section');panel.dataset.adminFinalReviewDocument='true';panel.className='ll-admin-final-review-document ll-admin-workspace-card wide';
  if(finalDoc?.storage_path){
    ...
    const direct=(data.documents||[])[0];
    panel.classList.add('is-direct');
...
function apply(){
  scheduled=false;
  if(route()==='contribute'){
    if(role()==='student')void refreshStudent(false);
    else if(role()==='teacher')teacherHandoff();
  }
}
