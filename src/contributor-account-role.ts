...
function roleSummary(role:ContributorRole){
  if(role==='teacher')return {
    kicker:'TEACHER / MENTOR ACCOUNT',
    title:'Your contributor account is for academic review.',
    body:'You can apply as a LitLab teacher reviewer, mentor assigned students, review their DOCX files, give rubric scores and testimony, and track mentoring history. Student contribution forms, CAS evidence and student certificates are kept out of this account.',
    badge:'Teacher reviewer',
    action:'Apply as a teacher reviewer'
  };
  return {
    kicker:'STUDENT CONTRIBUTOR ACCOUNT',
    title:'Your contributor account is for your own student work.',
    body:'You can apply as a student contributor, submit DOCX work, receive teacher or LitLab feedback, keep contribution evidence while active and receive recognition after approved work is completed. Teacher-reviewer applications are kept out of this account.',
    badge:'Student contributor',
    action:'Apply as a student contributor'
  };
}
...
function personalizePage(role:ContributorRole|null,blocked=false){
  ...
  if(applyHead&&role){
    ...
    if(h2)h2.textContent=role==='teacher'?'Apply to review and mentor student work.':'Tell us what you want to contribute.';
    if(p)p.textContent=role==='teacher'?'This account can only submit teacher / mentor applications. LitLab reviews teacher applications before any student work is assigned.':'This account can only submit student contributor applications. LitLab reviews each proposal before the contribution workflow begins.';
  }
...
async function chooseRole(role:ContributorRole,button:HTMLButtonElement){
  const panel=button.closest<HTMLElement>('[data-account-role-choice]');const status=panel?.querySelector<HTMLElement>('[data-account-role-status]');
  panel?.querySelectorAll<HTMLButtonElement>('button[data-choose-contributor-role]').forEach(b=>b.disabled=true);button.dataset.loading='true';
  ...
  try{
    const next=await rpc<RoleState>('set_my_litlab_contributor_account_role',{p_role:role});cached=next;applyState(next);window.dispatchEvent(new CustomEvent('litlab:contributor-account-role',{detail:next}));
    requestAnimationFrame(()=>applySection()?.scrollIntoView({block:'start',behavior:'smooth'}));
  }catch(error){
    const message=error instanceof Error?error.message:'Could not save the contributor role.';
    if(status){status.textContent=message;status.dataset.state='error'}
    panel?.querySelectorAll<HTMLButtonElement>('button[data-choose-contributor-role]').forEach(b=>b.disabled=false);delete button.dataset.loading;
  }
}
...
async function refresh(force=false){
  ...
  if(cached&&!force){applyState(cached);return}
  loading=true;const host=root();if(host)host.classList.add('ll-account-role-loading');
  try{cached=await rpc<RoleState>('get_my_litlab_contributor_account_role');applyState(cached);window.dispatchEvent(new CustomEvent('litlab:contributor-account-role',{detail:cached}))}
  catch(error){console.error('Contributor account role unavailable',error);personalizePage(null,true)}
  finally{loading=false;if(host)host.classList.remove('ll-account-role-loading')}
}

function scan(){
  window.clearTimeout(scanTimer);scanTimer=0;
  if(route()!=='contribute'){observer?.disconnect();observer=null;return}
  const host=root();const f=form();
  if(!host||!f){if(scanAttempts++<30)scanTimer=window.setTimeout(scan,100);return}
  scanAttempts=0;
  if(signedIn()){
    f.hidden=true;
    void refresh();
  }
  observer?.disconnect();observer=new MutationObserver(()=>{if(route()==='contribute'&&cached){applyState(cached)}});observer.observe(host,{childList:true,subtree:true});
}
