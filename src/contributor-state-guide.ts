...
type Assignment={application_id?:string;documents?:unknown[];reviews?:unknown[];status?:string};
type WorkspaceEvent={selectedId?:string;workspaces?:WorkspaceRow[];assignments?:Assignment[]};
type SectionAction='application';
type SectionLink={label:string;key:string;target?:HTMLElement|null;locked?:boolean;reason?:string;action?:SectionAction};
...
function stableKey(value:string){return value.toLowerCase().replace(/[^a-z0-9-]+/g,'-').replace(/^-+|-+$/g,'')||'section'}
function ensureId(el:HTMLElement,key:string){
  if(!el.id){
    const id=`ll-contributor-section-${stableKey(key)}`;
    const existing=document.getElementById(id);
    if(existing&&existing!==el)existing.removeAttribute('id');
    el.id=id;
  }
  el.dataset.contributorTocTarget='true';
  return el.id;
}
...
function journeySection(host:HTMLElement){return host.querySelector<HTMLElement>('[data-v3-journey]')}
function orientationSection(host:HTMLElement){return host.querySelector<HTMLElement>('[data-v3-standard]')}
...
function evidenceSection(host:HTMLElement){return host.querySelector<HTMLElement>('[data-clarity-evidence-primary],[data-v3-evidence]')||document.querySelector<HTMLElement>('[data-clarity-evidence-primary]')}
function historySection(host:HTMLElement){return host.querySelector<HTMLElement>('[data-v3-history],[data-contributor-completion-archive]')||document.querySelector<HTMLElement>('[data-my-contributions]')}
function impactSection(host:HTMLElement){return host.querySelector<HTMLElement>('[data-v3-impact]')}
function completionSection(host:HTMLElement){return host.querySelector<HTMLElement>('[data-lifecycle-complete-card],[data-contributor-completion-archive],.ll-recognition')}
...
function teacherZone(host:HTMLElement){return host.querySelector<HTMLElement>('[data-teacher-student-roster],[data-teacher-student-browser],.ll-teacher-zone,[data-teacher-mentor-dashboard]')}
...
function activeTeacherReviewForm(host:HTMLElement){return activeTeacherReview(host)?.querySelector<HTMLFormElement>('form[data-teacher-review],form.ll-review-form')||null}
...
function addAction(list:SectionLink[],label:string,key:string,action:SectionAction){
  if(list.some(item=>item.key===key))return;
  list.push({label,key,action});
}
...
function addStudentApplication(list:SectionLink[]){
  const active=activeStudentContribution();
  if(active){
    addLocked(list,'Application','application','Finish your current contribution before starting another. A new Student application unlocks after the active contribution is completed.');
    return;
  }
  addAction(list,'Application','application','application');
}
...
function flowInfo(activeRole:ContributorRole,host:HTMLElement|null,app:WorkspaceRow|null):{label:string;tone:Tone;flow:string}{
  ...
  if(activeRole==='student'){
    ...
    if(flow==='not-applied')return {label:'Student · ready to apply',tone:'neutral',flow};
    if(flow==='pending')return {label:'Student · application under review',tone:'waiting',flow};
    if(flow==='closed')return {label:'Student · application closed',tone:'closed',flow};
    if(flow==='working')return {label:count?'Student · contribution in progress':'Student · active contribution',tone:'action',flow};
    if(flow==='submitted')return {label:`Student · ${count} submission${count===1?'':'s'} in review`,tone:'waiting',flow};
    ...
    if(flow==='waiting-admin')return {label:'Student · final LitLab review',tone:'waiting',flow};
    return {label:'Student · contribution completed',tone:'success',flow};
  }
  const flow=teacherFlow(host,app);
  if(flow==='not-applied')return {label:'Teacher · ready to apply',tone:'neutral',flow};
  if(flow==='pending')return {label:'Teacher · application under review',tone:'waiting',flow};
  ...
  if(flow==='waiting-student')return {label:'Teacher · waiting for student DOCX',tone:'waiting',flow};
  if(flow==='caught-up')return {label:'Teacher · assigned reviews caught up',tone:'waiting',flow};
  return {label:'Teacher · approved and ready',tone:'success',flow};
}
...
function genericSections(){
  ...
  addAction(links,'Application','application','application');
  return links;
}
function studentSections(host:HTMLElement,app:WorkspaceRow|null){
  ...
  if(flow==='not-applied'){
    addOpen(links,'Overview',heroSection(),'overview');
    addOpen(links,'CAS',casSection(),'cas');
    addStudentApplication(links);
    addLocked(links,'Journey','journey','Your contribution journey appears after you submit an application.');
  ...
  addStudentApplication(links);
  addOpenOrLocked(links,'Journey',journeySection(host),'journey','Your contributor journey is still loading.');
  addOpenOrLocked(links,'Orientation',orientationSection(host),'orientation','The contributor orientation is still loading.');
  ...
  if(flow==='closed'){
    ...
    addOpenOrLocked(links,'History',historySection(host),'history','No saved contribution history is available yet.');
    addOpen(links,'Messages',messageSection(),'messages');
    return links;
  }
  ...
  if(visible(evidenceSection(host)))addOpen(links,'Evidence',evidenceSection(host),'evidence');
  else if(flow!=='completed')addLocked(links,'Evidence','evidence','Evidence tools are not available at this stage.');

  if(flow==='completed'){
    addOpenOrLocked(links,'Completion',completionSection(host),'completion','The completion record is loading.');
    addOpenOrLocked(links,'History',historySection(host),'history','The contribution history is loading.');
    if(visible(impactSection(host)))addOpen(links,'Impact',impactSection(host),'impact');
...
function teacherSections(host:HTMLElement,app:WorkspaceRow|null){
  ...
  addOpenOrLocked(links,'Journey',journeySection(host),'journey','Your teacher review journey is still loading.');
  addOpenOrLocked(links,'Orientation',orientationSection(host),'orientation','The teacher reviewer orientation is still loading.');

  if(flow==='pending'){
    addLocked(links,'Application','application','Your Teacher application is already submitted and under review. You do not need to apply again.');
  ...
  if(flow==='closed'){
    addLocked(links,'Application','application','This Teacher reviewer application is closed. Contact LitLab if you need clarification.');
  ...
  const reviewForm=activeTeacherReviewForm(host);
  if(visible(reviewForm))addOpen(links,'Review student',reviewForm,'review-student');
  else addLocked(links,'Review student','review-student',flow==='waiting-student'?'Waiting for the assigned student to upload a current DOCX.':flow==='caught-up'?'No student review is waiting right now.':'A review will unlock when LitLab assigns student work.');
...
function buttonMarkup(item:SectionLink){
  ...
  if(item.action)return `<button type="button" data-contributor-action="${esc(item.action)}" data-section-key="${esc(item.key)}">${esc(item.label)}</button>`;
...
function startObserver(){
  ...
  observer=new MutationObserver(records=>{
    const external=records.some(record=>{
      ...
      return !target?.closest('[data-contributor-state-guide]');
    });
    if(external)schedule();
  });
  observer.observe(document.body,{childList:true,subtree:true,attributes:true,attributeFilter:['hidden','data-contributor-account-role']});
}
function setCurrent(button:HTMLButtonElement|null){
  ...
  const buttons=Array.from(guide.querySelectorAll<HTMLButtonElement>('[data-contributor-section-jump],[data-contributor-action]'));
...
function syncCurrentFromScroll(){
  ...
  guide.querySelectorAll<HTMLButtonElement>('[data-contributor-section-jump]').forEach(button=>{
    const target=document.getElementById(button.dataset.contributorSectionJump||'');
    if(visible(target))entries.push({button,target});
  });
  const applicationButton=guide.querySelector<HTMLButtonElement>('[data-contributor-action="application"]');
  const application=applicationSection();
  if(applicationButton&&visible(application))entries.push({button:applicationButton,target:application});
  if(!entries.length){setCurrent(null);return}
  const marker=Math.max(110,Math.round(guide.getBoundingClientRect().bottom+12));
...
function showLockedReason(reason:string){
  ...
  toastTimer=window.setTimeout(()=>{toast.hidden=true;toast.textContent=''},4200);
}
function openApplicationFromGuide(){
  const activeRole=role();if(activeRole==='student'&&activeStudentContribution()){showLockedReason('Finish your current contribution before starting another. A new Student application unlocks after completion.');return}
  window.dispatchEvent(new CustomEvent('litlab:request-contributor-application',{detail:{role:activeRole||'student',source:'guide'}}));
  window.setTimeout(()=>{
    const apply=applicationSection();
    if(visible(apply)){
      apply.scrollIntoView({behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth',block:'start'});
      const focus=apply.querySelector<HTMLElement>('input:not([disabled]),select:not([disabled]),textarea:not([disabled]),button:not([disabled])');
      window.setTimeout(()=>focus?.focus({preventScroll:true}),280);
    }
  },90);
}
...
document.addEventListener('click',event=>{
  ...
  if(target.closest('[data-workspace-select],[data-teacher-student-select],[data-teacher-roster-student],[data-teacher-roster-mobile],summary'))window.setTimeout(schedule,0);
},true);
