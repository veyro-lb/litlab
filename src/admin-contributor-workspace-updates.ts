...
type Update={kind:'document'|'revision_response';update_id:string;application_id:string;created_at:string;full_name:string;topics:string;label:string};
...
function label(item:Update){return item.kind==='document'?`New DOCX from ${item.full_name}`:`Revision response from ${item.full_name}`}
