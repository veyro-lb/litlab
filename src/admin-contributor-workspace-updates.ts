...
function updateKey(item:Update){return `${item.kind}:${item.update_id}`}
function dismissed(){try{const raw=sessionStorage.getItem(NOTICE_KEY);if(!raw)return new Set<string>();const parsed=JSON.parse(raw);return new Set(Array.isArray(parsed)?parsed.map(String):[String(parsed)])}catch{const legacy=sessionStorage.getItem(NOTICE_KEY);return legacy?new Set([legacy]):new Set<string>()}}
function rememberDismissed(key:string){try{const values=[...dismissed()].filter(value=>value!==key);values.push(key);sessionStorage.setItem(NOTICE_KEY,JSON.stringify(values.slice(-40)))}catch{}}
function latestVisible(){const hidden=dismissed();return updates.slice().sort((a,b)=>Date.parse(b.created_at)-Date.parse(a.created_at)).find(item=>!hidden.has(updateKey(item)))||null}
...
function applyCards(){if(route()!=='admin-contributors')return;const byApp=new Map<string,Update>();updates.slice().sort((a,b)=>Date.parse(b.created_at)-Date.parse(a.created_at)).forEach(u=>{if(!byApp.has(u.application_id))byApp.set(u.application_id,u)});document.querySelectorAll<HTMLElement>('.admin-contrib-card[data-app-id]').forEach(card=>{const item=byApp.get(card.dataset.appId||'');card.classList.toggle('has-admin-workspace-update',Boolean(item));let badge=card.querySelector<HTMLElement>('[data-admin-workspace-update-badge]');if(item&&!badge){badge=document.createElement('span');badge.dataset.adminWorkspaceUpdateBadge='true';badge.className='admin-workspace-update-badge';card.querySelector('summary')?.appendChild(badge)}if(badge){if(item)badge.textContent=badgeLabel(item);else badge.remove()}const manage=card.querySelector<HTMLButtonElement>('[data-admin-manage-workspace]');if(manage)manage.classList.toggle('has-admin-workspace-update',Boolean(item))})}
function showNotice(){
  const item=latestVisible();if(!item)return;
  if(document.getElementById('ll-admin-contributor-update-notice')||document.getElementById('ll-admin-workspace-update-notice'))return;
  const key=updateKey(item);
  const n=document.createElement('aside');n.id='ll-admin-workspace-update-notice';n.className='ll-admin-workspace-update-notice';n.dataset.applicationId=item.application_id;n.dataset.updateKind=item.kind;n.dataset.title=item.full_name||item.topics||'Contributor workspace';n.innerHTML=`<button type="button" data-close aria-label="Dismiss">×</button><span>NEW CONTRIBUTOR UPDATE</span><div><i>●</i><section><b>${esc(label(item))}</b><p>${esc(item.topics||item.label)}</p></section></div><button type="button" data-open>Open this contributor →</button>`;
  const dismiss=()=>{rememberDismissed(key);n.classList.add('is-closing');setTimeout(()=>{n.remove();showNotice()},220)};
...
function apply(){applyAccountDot();applyMenu();applyCards();if(updates.length)showNotice();else document.getElementById('ll-admin-workspace-update-notice')?.remove()}
function clear(){updates=[];document.documentElement.removeAttribute('data-litlab-admin-workspace-update');document.querySelectorAll('[data-admin-workspace-menu-badge],[data-admin-workspace-update-badge]').forEach(el=>el.remove());document.getElementById('ll-admin-workspace-update-notice')?.remove()}

async function load(_force=false){if(!token()){isAdmin=null;clear();return}if(loading)return;loading=true;try{if(isAdmin===null)isAdmin=Boolean(await rpc<boolean>('is_litlab_admin'));if(!isAdmin){clear();return}const rows=await rpc<Update[]>('admin_get_litlab_contributor_unread_workspace_updates');updates=Array.isArray(rows)?rows:[];apply()}catch(error){console.debug('Admin workspace updates unavailable',error)}finally{loading=false}}
