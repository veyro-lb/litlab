...
function mount(){
 ...
 box.innerHTML=`<div class="p2-builder-intro"><div><span>✦ INTERACTIVE TOOL</span><h3>Build your Paper 2 comparison.</h3><p>Choose two works and a concept, then turn similarities, differences, methods, significance, and evidence into one comparative thesis and three paragraph arguments.</p><div class="p2-builder-flow"><b>WORK A</b><i>↔</i><b>WORK B</b><i>→</i><b>CONCEPT</b><i>→</i><strong>ARGUMENT</strong></div></div><button type="button" class="btn primary" data-builder-toggle>Build a comparison →</button></div><div class="p2-builder-workspace" data-builder-workspace hidden></div>`;
 section.append(box);
 ...
 let remoteTimer=0;
 const collect=()=>{workspace.querySelectorAll<HTMLInputElement|HTMLSelectElement|HTMLTextAreaElement>('[name]').forEach(el=>{const key=el.name as keyof BuilderState;if(key in state)state[key]=el.value});saveLocal(state);const status=workspace.querySelector<HTMLElement>('[data-builder-status],[data-builder-save]');if(status)status.textContent=session()?'Saving to your account…':'Saved on this device';window.clearTimeout(remoteTimer);remoteTimer=window.setTimeout(()=>void syncRemote(),650)};
 const syncRemote=async()=>{const s=session();if(!s)return;try{const user=await currentUser(s);if(!user)return;await saveRemote(s,user,state);workspace.querySelectorAll<HTMLElement>('[data-builder-status],[data-builder-save]').forEach(x=>x.textContent='Synced to your LitLab account ✓')}catch{workspace.querySelectorAll<HTMLElement>('[data-builder-status],[data-builder-save]').forEach(x=>x.textContent='Saved on this device • account sync will retry') }};
 ...
 if(Object.values(state).some(Boolean)){workspace.hidden=false;renderWorkspace()}
