...
type Update={kind:'brief'|'task'|'revision'|'teacher_assignment';update_id:string;application_id:string;created_at:string;topics:string;label:string};
...
function title(item:Update){if(item.kind==='brief')return 'LitLab updated your project brief';if(item.kind==='task')return 'You have a new task or task update';if(item.kind==='revision')return 'LitLab updated a revision request';return 'You have a new teacher review assignment'}
