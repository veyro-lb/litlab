...
function showNotice(){
  ...
  const n=document.createElement('aside');n.id='ll-user-workspace-update-notice';n.className='ll-user-workspace-update-notice';n.innerHTML=`<button type="button" data-close aria-label="Dismiss">×</button><span>${item.kind==='certificate'?'CERTIFICATE READY':'NEW CONTRIBUTOR UPDATE'}</span><div><i>●</i><section><b>${esc(title(item))}</b><p>${esc(item.topics||item.label)}${item.label?` • ${esc(item.label)}`:''}</p></section></div><button type="button" data-open>${item.kind==='certificate'?'Open & download certificate →':'Open my contributor workspace →'}</button>`;
...
document.addEventListener('click',event=>{const target=event.target instanceof Element?event.target:null;if(target?.closest('.litlab-account-trigger'))setTimeout(apply,180)},true);
window.addEventListener('hashchange',()=>{if(route()==='contribute')clear();setTimeout(()=>void load(true),250)});
