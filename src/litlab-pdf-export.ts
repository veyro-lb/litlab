...
class EvidenceRenderer{
  ...
  private newPage(first=false){
    ...
    ctx.strokeStyle=LINE;ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(64,132);ctx.lineTo(PORTRAIT.pxWidth-64,132);ctx.stroke();this.y=172;if(first)this.coverIntro();
  }
  private coverIntro(){
    const {ctx}=this.page;font(ctx,44,900);ctx.fillStyle=INK;const title=wrap(ctx,this.data.contributionTitle,this.contentWidth).slice(0,4);title.forEach((line,i)=>ctx.fillText(line,this.margin,this.y+i*51));this.y+=Math.max(1,title.length)*51+18;
    font(ctx,18,650);ctx.fillStyle=MUTED;ctx.fillText(this.data.contributorName,this.margin,this.y);this.y+=34;font(ctx,13,700);ctx.fillStyle=PURPLE;ctx.fillText(this.data.contributionType.toUpperCase(),this.margin,this.y);this.y+=38;
  ...
  private row(row:EvidenceRow){
    let ctx=this.page.ctx;font(ctx,14,550);let remaining=wrap(ctx,row.value,this.contentWidth-282),first=true;
    while(remaining.length){this.ensure(70);ctx=this.page.ctx;font(ctx,14,550);const available=Math.max(1,Math.floor((this.bottom-this.y-24)/23)),chunk=remaining.splice(0,available),height=Math.max(48,chunk.length*23+22);font(ctx,11,850);ctx.fillStyle=MUTED;ctx.fillText(`${row.label.toUpperCase()}${first?'':' (CONTINUED)'}`,this.margin,this.y+19);font(ctx,14,550);ctx.fillStyle=INK;chunk.forEach((line,i)=>ctx.fillText(line,this.margin+282,this.y+19+i*23));ctx.strokeStyle='#edf0f5';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(this.margin,this.y+height);ctx.lineTo(this.margin+this.contentWidth,this.y+height);ctx.stroke();this.y+=height+6;first=false;if(remaining.length)this.newPage(false)}
  }
