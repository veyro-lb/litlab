...
function polishAuthCopy(){
  ...
  if(dialog){
    ...
    if(intro&&intro.textContent?.includes('Use your Google account')){
      intro.textContent='Choose an available sign-in method to create or access your LitLab account. You can also close this window and keep browsing.';
    }
    const note=dialog.querySelector<HTMLElement>(':scope > small');
    if(note&&note.textContent?.includes('Google password')){
      note.textContent='Your sign-in password is handled by your provider and is never shared with LitLab.';
    }
  }

  document.querySelectorAll<HTMLElement>('.litlab-account-status b').forEach(element=>{
    if(element.textContent?.trim()==='Google account connected')element.textContent='Account connected';
  });
}
...
function polishAccountCenter(){
  ...
  const connected=card.querySelector<HTMLElement>('.litlab-account-center-head i');
  if(connected?.textContent?.trim()==='Google connected')connected.textContent='Account connected';

  const identity=card.querySelector<HTMLElement>('.litlab-account-center-head p');
  if(identity?.textContent?.trim()==='Google account')identity.textContent='Signed-in account';
  ...
  if(message?.textContent?.includes('Your Google password and other Google data')){
    message.textContent='Only LitLab learning progress is included. Your sign-in password and external account data are never stored by LitLab.';
  }
}

function polishAdminCopy(){
  document.querySelectorAll<HTMLElement>('.admin-user-table th').forEach(element=>{
    if(element.textContent?.trim()==='Last Google sign-in')element.textContent='Last provider sign-in';
  });
  ...
  document.querySelectorAll<HTMLElement>('.admin-metric small,.admin-gate-card p,.admin-privacy p').forEach(element=>{
    ...
    const next=element.textContent
      ...
      .replace('No Google passwords, Gmail messages, Drive files','No provider passwords, email messages, cloud files');
    if(next!==element.textContent)element.textContent=next;
  });
}

function polishDashboard(){
  const local=document.querySelector<HTMLElement>('.my-litlab-local');
  if(local?.textContent?.includes('no account'))local.textContent='Saved on this browser';
}
...
function polishBookProfile(){
  ...
  const placeholder=bookSearchExamples[title];
  const input=page.querySelector<HTMLInputElement>('[data-moment-search]');
  if(placeholder&&input&&input.placeholder!==placeholder)input.placeholder=placeholder;
}
...
const relevantSelector=[
  '.litlab-auth-dialog','.litlab-account-menu','.litlab-account-center-card',
  '.admin-page','.admin-gate','.my-litlab-dashboard','.books-profile-page'
].join(',');
...
const observer=new MutationObserver(mutations=>{
  for(const mutation of mutations){
    for(const node of mutation.addedNodes){
      if(relevantNode(node)){schedule();return}
    }
  }
});
