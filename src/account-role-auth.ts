...
async function rpc<T>(name:string,body:Record<string,unknown>={}):Promise<T>{
  ...
  try{
    ...
    if(!response.ok){
      ...
      try{const json=await response.json() as {message?:string};if(json.message)message=json.message}catch{}
...
function roleName(state:RoleState|null){
  if(state?.is_admin)return 'Admin';
  if(state?.role==='teacher')return 'Teacher / mentor';
  if(state?.role==='student')return 'Student';
  return 'Not selected';
}

function roleDescription(state:RoleState|null){
  if(state?.is_admin)return 'LitLab admin account';
  if(state?.role==='teacher')return 'Teacher review and mentoring account';
  if(state?.role==='student')return 'Student learning and contributor account';
  return 'Choose Student or Teacher after sign-in';
}

function decorateSignInModal(){
  const dialog=document.querySelector<HTMLElement>('[data-auth-modal] .litlab-auth-dialog');
  if(!dialog||dialog.querySelector('[data-auth-role-info]'))return;
  const authButton=dialog.querySelector<HTMLElement>('[data-auth-google]')||dialog.querySelector<HTMLElement>('button');
  if(!authButton)return;
  const info=document.createElement('div');
  info.className='ll-auth-role-info';
  info.dataset.authRoleInfo='true';
...
function decorateAccountMenu(){
  document.querySelectorAll<HTMLElement>('.litlab-account-menu').forEach(menu=>{
    ...
    if(!row){
      ...
      row.className='ll-account-type-row';
      row.dataset.accountTypeRow='true';
      const divider=menu.querySelector('.litlab-account-divider');
      if(divider)divider.before(row);else menu.appendChild(row);
    }
    const name=roleName(roleState);
    const detail=roleDescription(roleState);
    const unselected=!roleState?.is_admin&&!roleState?.role;
    row.innerHTML=`<div class="ll-account-type-icon">${roleState?.is_admin?'A':roleState?.role==='teacher'?'✓':'✦'}</div><div><small>ACCOUNT TYPE</small><b>${name}</b><span>${detail}</span></div>${unselected?'<button type="button" data-open-account-role>Choose</button>':''}`;
...
function closeRoleChooser(){
  document.querySelector('[data-auth-role-setup]')?.remove();
  document.documentElement.classList.remove('ll-auth-role-open');
}
...
function openRoleChooser(state:RoleState){
  ...
  closeRoleChooser();
  document.body.insertAdjacentHTML('beforeend',chooserMarkup(state));
  document.documentElement.classList.add('ll-auth-role-open');
...
async function chooseRole(role:ContributorRole,button:HTMLButtonElement){
  const overlay=document.querySelector<HTMLElement>('[data-auth-role-setup]');
  const status=overlay?.querySelector<HTMLElement>('[data-auth-role-status]');
  overlay?.querySelectorAll<HTMLButtonElement>('[data-auth-choose-role]').forEach(item=>item.disabled=true);
  button.dataset.loading='true';
  if(status)status.textContent=`Saving ${role==='teacher'?'Teacher / mentor':'Student'} as your account type…`;
  try{
    ...
    const message=error instanceof Error?error.message:'Could not save your account type.';
    if(status)status.textContent=message;
    overlay?.querySelectorAll<HTMLButtonElement>('[data-auth-choose-role]').forEach(item=>item.disabled=false);
    delete button.dataset.loading;
  }
}

function applyRoleState(state:RoleState){
  roleState=state;
  decorateAccountMenu();
  if(state.is_admin||state.role){closeRoleChooser();return}
  if(state.needs_choice||state.has_conflict||!state.role)openRoleChooser(state);
}
...
window.addEventListener('litlab:contributor-account-role',event=>{
  const next=(event as CustomEvent<RoleState>).detail;
  if(!next||typeof next!=='object')return;
  roleState=next;decorateAccountMenu();if(next.is_admin||next.role)closeRoleChooser();
});
